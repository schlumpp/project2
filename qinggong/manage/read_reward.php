...
<?php
	...
	$sid=$_GET["sid"];
	$year=date('Y');
	$sql="select * from  national_scholarship  join  student  on  national_scholarship.stu_id=student.stu_id   where  national_scholarship.nationalSch_year  ='".$year."' and  student.stu_id =".$sid."";
	...
	echo" <th> 成绩排名:</th>";
	echo"<td> ".$row['ranking1']."</td>";
	echo" <th> 综合排名:</th>";
	echo"<td> ".$row['ranking2']."</td>";
	echo "</tr>" ;	
	
	echo" <th> 主修课程门数:</th>";
	echo"<td> ".$row['Com_Course']."</td>";
	echo" <th> 通过课程门数:</th>";
	echo"<td> ".$row['pass_num']."</td>";
	echo" <th> 申请理由:</th>";
	echo"<td> ".$row['nationalSch_applyReason']."</td>";
	echo "</tr>" ;	
