...
/*if(!isset($_SESSION["managename"]))
 ...
 {*/
 $year=date('Y');
/*  left join national_scholarship  on  national_scholarship.stu_id=apply_id*/
	   $sql="select * from student left join applyfor on   student.stu_id= applyfor.apply_id   where student.type=2 and applyfor.applyfor_year ='".$year."' and applyfor.apply_type =2 and applyfor.applySch_state=1  limit $offset,$Page_size";    
      $rs=mysql_query($sql);
	  ?>
	 ...
	 while($row=mysql_fetch_array($rs))
	 {
		...
		if($num%2==1)
		   ...
		   print("<tr>");
	    $num=$num+1;
		 print("<td>".$num."</td><td> ".$row['stu_num']." </td><td>".$row['stu_name']."</td><td>".$row['stu_class']."</td><td>".$row['stu_hometown']."</td>");	
		print("<td>"."<a href='read_reward.php?sid=".$row["stu_id"]."'target'='_blank'><img src='img/view.gif' border='0'/>"."</td>");
