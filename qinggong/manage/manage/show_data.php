...
/*if(!isset($_SESSION["managename"]))
 ...
 {*/
	  $sql="select * from data limit $offset,$Page_size";
      $rs=mysql_query($sql);
	 print("<center>");
	 print("<table   width='70%'>");
	 print("<tr  class='f_tr'>");
	 print("<td><h3> 编名    </h3></td>");
	 print("<td><h3> 上传者    </h3></td>");
	 print("<td><h3> 资料名称 </h3></td>");
	 print("<td><h3>   上传时间 </h3></td>");
	 print("<td><h3> 下载次数 </h3></td>");
	 ...
	 while($row=mysql_fetch_array($rs))
	 {
	    ...
	    $num=$num+1;
		 print("<td>".$num."</td><td><center>".$row['da_publish']."</a></center></td><td>".$row['da_name']."</td><td>".$row['da_time']."</td><td>".$row['da_num']."</td>");	
		 print("<td>"."<a href=\"javascript:if(confirm('是否要删除？')) window.location='delete_data.php?sid=".$row['da_id']."';\"><img src='img/delete.gif' border='0'/>"."</td>");
		   print("</tr>");
