...
/*if(!isset($_SESSION["managename"]))
 ...
 {*/
 $year=date('Y');
	 $sql="select * from student left join applyfor on   student.stu_id= applyfor.apply_id left join national_scholarship  on  national_scholarship.stu_id=apply_id   where student.type=2 and applyfor.applyfor_year ='".$year."' and applyfor.apply_type =3 and applyfor.applySch_state=2 limit $offset,$Page_size";  
      $rs=mysql_query($sql);
	 ...
	 print("<td><h3><br/>身份证号 </h3></td>");
	 print("<td><h3><br/>查看 </h3></td>");
	 print("</tr>");
	 while($row=mysql_fetch_array($rs))
	 {
		...
		if($num%2==1)
		   ...
		   print("<tr>");
	    $num=$num+1;
		 print("<td>".$num."</td><td><center>".$row['stu_num']."</center></td><td>".$row['stu_name']."</td><td>".$row['stu_class']."</td><td>".$row['stu_hometown']."</td><td>".$row['stu_idCard']."</td>");	
