...
class CreateWordByPHP 
{
	...
	{  
		...
		if($row!=1  )
		{
		  ...
		  $template->save("D:\wamp\www\hnxy\manage\php\CreateWordByPHP\\error\\".$sql[1].".docx");
  			return;
		}
		//记录正确，读取信息，生成word
	   ...
	   //学生基本信息,查询学生表
		...
		if($row!=1 )
		{
			...
			$template->save("D:\wamp\www\hnxy\manage\php\CreateWordByPHP\\error\\".$myrow[1].".docx");
  			return ;
		}
		//记录正确，读取信息，生成word
		...
		$t=time();
		$year = date("Y",$t);
		$template->save("D:\wamp\www\hnxy\manage\php\CreateWordByPHP\\nation\\".$year.$myrow[1].".docx");
	}
	...
	{   //记录正确，读取信息，生成word
		...
		{
  			echo "记录多余或少于一条，有错误";
			$template->save("D:\wamp\www\hbxy\manage\php\CreateWordByPHP\\error\\".$sql[1].".docx");
  			return ;
		}
...
//存储
		$t=time();
		$year = date("Y",$t);
		$template->save("D:\wamp\www\hnxy\manage\php\CreateWordByPHP\\aid\\".$year.$myrow[1].".docx");
	}
}
