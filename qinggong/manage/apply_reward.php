...
<?php 
					...
					$sql = "select count(*) as amount from student left join applyfor on   student.stu_id= applyfor.apply_id   where student.type=2 and applyfor.applyfor_year ='".$year."' and applyfor.apply_type =2 and applyfor.applySch_state=1 ";
				//	print $sql;
					$rs=mysql_query($sql);
...
<div id="contentWrap">
  ...
  <div id="widget table-widget">
    <div class="pageTitle">申请奖学金学生</div>
    <div class="pageColumn">
      ...
      <table>
        ...
        <tbody>
		<form  action="deal_poorstu.php?action=2" method="post">
	<?php	
		              if($amount )
                        {
                

                        $query=mysql_query("select * from student left join applyfor on   student.stu_id= applyfor.apply_id   where student.type=2 and applyfor.applyfor_year ='".$year."' and applyfor.apply_type =2 and applyfor.applySch_state=1  order by  student.stu_id  limit ". ($page-1)*$page_size .", $page_size");
