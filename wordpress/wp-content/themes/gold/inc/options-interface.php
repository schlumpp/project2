...
function optionsframework_tabs() {
	...
	foreach ( $options as $value ) {
		...
		if ( $value['type'] == "heading" ) {
			...
			$menu .= '<li><a class="current" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '" data-toggle="tab">' . esc_html( $value['name'] ) . '</a></li>';
		}
	}
	...
	foreach ( $options as $value ) {
		...
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
			...
			if ( isset( $value['name'] ) ) {
				...
				if($value['name']=='Video ID (See image below)'){
					$v=$value['name'];
				}
			}
		...
		case 'text':
			...
			if(($v!='') && ($value['id']=='show_video_hidden')){
				$path=get_bloginfo('template_directory').'/img/video.png';
				$output .= '<img src="'.$path.'">';	
			}
			break;
		...
		case 'select':
			...
			$output .= '</select>';
			if($value['id']=='ycategory'){
				$output.='<a class="ycategory-link" href="'.admin_url().'edit-tags.php?taxonomy=category">Add New Category Here</a>';
			}
			break;
