...
class qq
{
	...
	function get_user_info() {
		$get_user_info = "https://graph.qq.com/user/get_user_info?" . "access_token=" . $_SESSION ['access_token'] . "&oauth_consumer_key=" . get_option('qqid') . "&openid=" . $_SESSION ['openid'] . "&format=json";		
		return get_url_contents ( $get_user_info );
	}
	function qq_cb(){
		global $wpdb;
		$openid_db = $wpdb->get_col("SELECT openid FROM $wpdb->users");
	    if (in_array($_SESSION['openid'], $openid_db)) {
	        $user_ID = $wpdb->get_results("SELECT ID FROM $wpdb->users WHERE openid='$_SESSION[openid]'");
	...
	function qq_bd(){
	    ...
	    if ($data) {
	        wp_redirect(get_page_slug_link('m'));
	        exit();
	    }else{
