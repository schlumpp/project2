...
class sina 
{
	...
	function sina_cb(){
		global $wpdb;
		$openid_db = $wpdb->get_col("SELECT uid FROM $wpdb->users");
		$user_ID = $wpdb->get_results("SELECT ID FROM $wpdb->users WHERE uid='$_SESSION[uid]'");
	...
	function sina_bd(){
		...
		$userid = $userinfo->id;
		$wpdb->query("UPDATE $wpdb->users SET uid = '$_SESSION[uid]' WHERE ID = $userid");
